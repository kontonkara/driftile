...
const MAX_GAP = 64;
const MIN_GAP = 0;
const MAX_MANUAL_FLOATING_SIZE_SETTLEMENT_PROBES = 20;
...
interface DesktopSendFocusTarget {
  readonly id: WindowId;
  readonly layer: WindowLayer;
  readonly window: KWinWindow;
}
...
type ManualFloatingResizeAxis = "height" | "width";

interface PendingManualFloatingSizeChange {
  readonly axis: ManualFloatingResizeAxis;
  readonly command: ManualFloatingFrameCommand;
  readonly constraintBounds: FrameSizeConstraintBounds;
  readonly decorationExtent: number;
...
interface FreshInitialFocusPolicy {
  readonly defaultFocus: DefaultInitialFocus;
  readonly focused: ApplicationInitialFocused;
...
export class RuntimeController {
  ...
  >();
  private readonly initialFocusPolicyByWindow = new Map<
    WindowId,
    FreshInitialFocusPolicy
  >();
  ...
  >();
  private readonly initialMaximizedPolicyByWindow = new Map<
    WindowId,
  ...
  private readonly pendingManualFloatingSizeChanges = new Map<
    WindowId,
  ...
  private pointerExternalPreviewLease: PointerPreviewContextLease<PointerExternalPreviewContext | null> | null =
    null;
  private pointerExternalPreviewPresentedContextKey: string | null = null;
  ...
  private pointerDropPreviewScopeToken: string | null = null;
  ...
  constructor(workspace: KWinWorkspace, options: RuntimeControllerOptions) {
    ...
    this.defaultInitialFocus =
      options.defaultInitialFocus ?? DEFAULT_INITIAL_FOCUS;
  ...
  setDefaultInitialFocus(focus: DefaultInitialFocus): boolean {
    if (this.defaultInitialFocus === focus) {
      return false;
    }

    this.defaultInitialFocus = focus;
    return true;
  }
  ...
  setApplicationInitialMaximized(
    ...
    this.applicationInitialMaximized = applications;
    return true;
  }
  ...
  sendWindowToPreviousDesktop(): boolean {
    return this.moveActiveWindowToDesktop(
      { direction: -1, kind: "adjacent" },
      false,
      false,
  ...
  sendWindowToNextDesktop(): boolean {
    return this.moveActiveWindowToDesktop(
      { direction: 1, kind: "adjacent" },
      false,
      false,
  ...
  sendWindowToDesktop(index: number): boolean {
    if (!validDesktopIndex(index)) {
      return false;
    }
  ...
  sendColumnToPreviousDesktop(): boolean {
    return this.moveActiveWindowToDesktop(
      { direction: -1, kind: "adjacent" },
      true,
      false,
  ...
  sendColumnToNextDesktop(): boolean {
    return this.moveActiveWindowToDesktop(
      { direction: 1, kind: "adjacent" },
      true,
      false,
  ...
  sendColumnToDesktop(index: number): boolean {
    if (!validDesktopIndex(index)) {
      return false;
    }
  ...
  reconcile(): number {
    ...
    this.finishPendingInitialAdmissionStates();
  ...
  private readonly handleCurrentDesktopChanged = (
    ...
    this.invalidatePointerExternalPreviewForDesktopSelection(output);
    ...
    if (liveCurrent) {
      ...
      for (const name of outputNames) {
        ...
        this.desktopFocusCaptureContexts.set(outputId(name), key);
  ...
  private readonly handleWindowTracked = (id: string): void => {
    ...
    if (
      this.initialWindowDiscoveryComplete &&
      source?.normalWindow &&
      (this.applicationInitialFocused.canonicalEntries.length > 0 ||
        this.applicationInitialUnfocused.canonicalEntries.length > 0 ||
        this.defaultInitialFocus !== DEFAULT_INITIAL_FOCUS)
    ) {
      const policy = Object.freeze({
        defaultFocus: this.defaultInitialFocus,
        focused: this.applicationInitialFocused,
        unfocused: this.applicationInitialUnfocused,
      });
      this.initialFocusPolicyByWindow.set(trackedId, policy);
    }

    if (
      this.initialWindowDiscoveryComplete &&
      source?.normalWindow &&
      this.applicationInitialMaximized.canonicalEntries.length > 0
    ) {
      this.initialMaximizedPolicyByWindow.set(
        trackedId,
        this.applicationInitialMaximized,
      );
    }
  ...
  private readonly handlePointerDropPreviewCursorChanged = (): void => {
    ...
    if (!intent || intent.phase !== "dragging") {
      return;
    }
  ...
  private schedulePointerDropPreview(): void {
    if (
      this.pointerDropPreviewUnavailable ||
      !this.showPointerDropPreview ||
      !this.hidePointerDropPreview ||
      this.pointerDropPreviewScheduleToken !== null
    ) {
      return;
    }

    const token = {};
    this.pointerDropPreviewScheduleToken = token;

    try {
      this.schedule(() => {
        if (this.pointerDropPreviewScheduleToken !== token) {
          return;
        }

        this.pointerDropPreviewScheduleToken = null;
        this.updatePointerDropPreview();
      });
    } catch (error) {
      if (this.pointerDropPreviewScheduleToken === token) {
        this.pointerDropPreviewScheduleToken = null;
  ...
  private updatePointerDropPreview(): void {
    ...
    const external = intent.floating
      ? null
      : this.resolveExternalPointerDrop(intent, point);
  ...
  private updateExternalPointerDropPreview(
    ...
    if (!lease) {
      ...
      lease = this.pointerExternalPreviewContexts.acquire(
        {
    ...
    if (
      capture?.lease !== lease ||
      !this.pointerExternalPreviewContainsCursor(capture, cursor)
    ) {
    ...
    if (
      ...
      this.pointerExternalPreviewPresentedContextKey = external.contextKey;
    }
  }
  ...
  private reusableExternalPointerPreviewLease(
    external: PointerExternalDropIntent,
  ): PointerPreviewContextLease<PointerExternalPreviewContext | null> | null {
    ...
    if (
      !lease ||
      !preview ||
      !this.pointerExternalPreviewContexts.owns(lease) ||
      lease.key.outputId !== external.context.outputId ||
      lease.key.desktopId !== external.context.desktopId ||
      lease.key.activityId !== external.context.activityId ||
      lease.key.topologyRevision !== this.topologyRevision ||
      ...
      this.waitingWindowIds.has(key) ||
      this.toggleTransitionPending(key) ||
      this.contexts.get(key) !== preview.runtimeContext ||
      preview.runtimeContext.geometryFingerprint !==
        lease.key.geometryFingerprint
    ) {
      return null;
    }
  ...
  private pointerExternalPreviewContainsCursor(
    capture: PointerExternalPreviewCapture,
    cursor: Point,
  ): boolean {
    if (capture.preview.kind === "window") {
      return rectContainsPoint(capture.preview.frame, cursor);
    }

    const frame = capture.preview.frame;
    return (
      rectContainsPoint(capture.context.visibleArea, cursor) &&
      cursor.x >= frame.x &&
  ...
  private showPointerDropPreviewFrame(
    ...
    if (
      ...
      this.pointerDropPreviewScopeToken === scopeToken
    ) {
      return true;
    }
    ...
    const ownerToken = this.nextPointerDropPreviewOwnerToken();
  ...
  private clearPointerDropPreview(): void {
    if (this.pointerDropPreviewFrame === null) {
      return;
    }

    const conflictingOutline = this.pointerDropPreviewHasConflictingOutline();
  ...
  private disablePointerDropPreview(error: unknown): void {
    const hidePreview =
      this.pointerDropPreviewFrame !== null &&
      !this.pointerDropPreviewHasConflictingOutline();
    this.pointerDropPreviewUnavailable = true;
    this.pointerDropPreviewScheduleToken = null;
    ...
    console.warn(
      `[driftile] pointer drop preview disabled error=${String(error)}`,
    );
  }

  private pointerDropPreviewHasConflictingOutline(): boolean {
    const frame = this.pointerDropPreviewFrame;

    for (const window of this.workspace.stackingOrder) {
      if (
        window.outline === true &&
        (frame === null || !rectsEqual(window.frameGeometry, frame))
      ) {
        return true;
      }
    }
  ...
  private resetPointerDropPreviewState(): void {
    ...
    this.pointerDropPreviewScopeToken = null;
  }
  ...
  private invalidatePointerExternalPreviewContext(
    ...
    if (
      preserveExpectedSourceTransition &&
      this.pointerExternalPreviewOwnsExpectedSourceTransition(key)
    ) {
      return;
    }
  ...
  private pointerExternalPreviewOwnsExpectedSourceTransition(
    key: string,
  ): boolean {
    const intent = this.pointerMoveIntent;
    ...
    if (
      ...
      intent.floating ||
      !capture ||
      capture.context.runtimeContext.key !== key ||
      this.workspace.activeWindow !== intent.source ||
      !intent.source.move
    ) {
      return false;
    ...
    const observed = normalizeWindow(intent.source);
    const liveContext = observed ? this.resolveManagedContext(observed) : null;

    if (!liveContext || contextKey(liveContext) !== key) {
      return false;
    }

    const participantIds = new Set(
      capture.context.participants.map((participant) => participant.id),
    );
    const members = capture.context.runtimeContext.windowIds;

    return (
      capture.context.participants.every((participant) =>
        members.has(participant.id),
      ) &&
      [...members].every(
        (id) => id === intent.draggedWindowId || participantIds.has(id),
      )
    );
  }

  private invalidatePointerExternalPreviewForDesktopSelection(
    output?: KWinOutput,
  ): void {
    const lease = this.pointerExternalPreviewLease;
    const emptyLease = this.pointerExternalEmptyPreviewLease;
    ...
    const destination = managedContextFromKey(key);

    if (
      output &&
      destination &&
      destination.outputId !== outputId(output.name)
    ) {
      return;
    }

    this.invalidatePointerExternalPreviewContext(key);
  }

  private startPointerDropPreviewTracking(): void {
    if (
      this.pointerDropPreviewCursorConnected ||
      !this.showPointerDropPreview ||
      !this.hidePointerDropPreview ||
      !this.workspace.cursorPosChanged
    ) {
      return;
    }

    this.workspace.cursorPosChanged.connect(
      this.handlePointerDropPreviewCursorChanged,
    );
    this.pointerDropPreviewCursorConnected = true;
  }

  private stopPointerDropPreviewTracking(): void {
    this.pointerDropPreviewScheduleToken = null;

    if (!this.pointerDropPreviewCursorConnected) {
      return;
    }

    this.workspace.cursorPosChanged?.disconnect(
      this.handlePointerDropPreviewCursorChanged,
    );
    this.pointerDropPreviewCursorConnected = false;
  }
  ...
  private prepareExternalPointerDrop(
    ...
    const settled = this.settledExternalPointerContext(external);
    if (typeof settled === "string") {
      external.insertion = { state: settled };
      return external;
    }

    const captured = this.captureExternalPointerInsertionFromPreview(
      intent,
      external,
      cursor,
      settled,
    );
    external.insertion = captured
      ? captured
      : this.pointerExternalPreviewPresentedContextKey === external.contextKey
        ? { state: "unavailable" }
  ...
  private resolveExternalPointerDrop(
    ...
    for (const candidate of this.workspace.screens) {
      if (!rectContainsPoint(candidate.geometry, cursor)) {
        continue;
      }
  ...
  private captureExternalPointerInsertionFromPreview(
    ...
    if (
      ...
      !this.externalPointerPreviewParticipantsAreCurrent(
        capture.context,
        intent.draggedWindowId,
      )
    ) {
  ...
  private externalPointerPreviewParticipantsAreCurrent(
    ...
    const participantIds = new Set(
      preview.participants.map((participant) => participant.id),
    );
    const membersAreCurrent =
      preview.participants.every((participant) =>
        context.windowIds.has(participant.id),
      ) &&
      [...context.windowIds].every(
        (id) => id === draggedWindowId || participantIds.has(id),
      );
    ...
    if (
      ...
      this.pointerContextHasPendingSync(
  ...
  private readonly handleWindowStateChanged = (id: string): void => {
    ...
    if (
      ...
      this.invalidatePointerExternalPreviewContext(
        previewContext.runtimeContext.key,
      );
    }
  ...
  private readonly handleWindowRemoved = (id: string): void => {
    ...
    this.cancelPendingManualFloatingSizeChange(managedId);
    this.cancelInvalidPendingExpelFocusHandoff();
  ...
  private desktopSendFocusTarget(
    key: string,
    sourceLayout: LayoutContextSnapshot,
    movingIds: ReadonlySet<WindowId>,
  ): DesktopSendFocusTarget | null {
    const activeIndex = sourceLayout.columns.findIndex(
      (column) => column.id === sourceLayout.activeColumnId,
    );
    const orderedColumns =
      activeIndex < 0
        ? sourceLayout.columns
        : [
            sourceLayout.columns[activeIndex],
            ...sourceLayout.columns.slice(0, activeIndex),
            ...sourceLayout.columns.slice(activeIndex + 1),
          ];
    ...
    for (const column of orderedColumns) {
      ...
      for (const id of [
        ...
        ...column.windowIds.filter((id) => id !== column.selectedWindowId),
      ]) {
        const candidate = this.desktopSendFocusCandidate(id, key, movingIds);

        if (candidate?.layer === "tiling") {
          return candidate;
        }
      }
    }

    let floating: DesktopSendFocusTarget | null = null;

    for (const id of this.windowRemovalFocusHistory) {
      const candidate = this.desktopSendFocusCandidate(id, key, movingIds);

      if (candidate?.layer === "floating") {
        floating = candidate;
      }
    }
    ...
    for (const id of this.contexts.get(key)?.windowIds ?? []) {
      const candidate = this.desktopSendFocusCandidate(id, key, movingIds);
  ...
  private desktopSendFocusCandidate(
    id: WindowId,
    key: string,
    movingIds: ReadonlySet<WindowId>,
  ): DesktopSendFocusTarget | null {
    const window = movingIds.has(id) ? undefined : this.observer.source(id);
    const layer = window
      ? this.windowRemovalFocusCandidateLayer(id, window, key)
      : null;

    return window && layer ? { id, layer, window } : null;
  }

  private focusDesktopSendSource(
    ...
    if (target) {
      ...
      try {
        this.workspace.activeWindow = target.window;
      } catch {
        focusRequestFailed = true;
      } finally {
        this.focusRequestDepth -= 1;
      ...
      if (
        focusRequestFailed ||
        this.workspace.activeWindow !== target.window ||
        this.observer.source(target.id) !== target.window ||
        this.windowRemovalFocusCandidateLayer(
          target.id,
          target.window,
          sourceContextKey,
    ...
    return !focusRequestFailed && this.workspace.activeWindow === null;
  }
  ...
  private takePendingDesktopFocus(
    ...
    if (this.desktopFocusCaptureContexts.get(liveContext.outputId) === key) {
  ...
  private resizeActiveManualFloatingWindowSize(
    ...
    let decorationExtent: number | null;

    try {
      constraintBounds = frameSizeConstraintBounds(activeWindow);
      decorationExtent = validDecorationExtent(
        command.originalFrame[axis],
        activeWindow.clientGeometry[axis],
      );
    } catch {
      return false;
    }

    if (!signal || !constraintBounds || decorationExtent === null) {
      return false;
    }

    const targetFrame = this.manualFloatingSizeTarget(
      axis,
      command,
      constraintBounds,
      decorationExtent,
    ...
    try {
      ...
      this.pendingManualFloatingSizeChanges.delete(activeId);
      operation.status = "rejected";
      console.warn(
        `[driftile] floating ${axis} resize signal connection failed window=${String(activeId)} error=${String(error)}`,
      );
      return false;
    }

    if (this.pendingManualFloatingSizeChanges.get(activeId) !== operation) {
      return false;
    }
    ...
    try {
      forwardWrites = this.geometry.apply(
        ...
        command.context,
        () =>
          this.pendingManualFloatingSizeChangeIsCurrent(operation) &&
    ...
    if (forwardWrites !== 1) {
      this.finishPendingManualFloatingSizeChange(operation, "rejected");

      if (forwardError !== null) {
        console.warn(
          `[driftile] floating ${axis} resize request failed window=${String(activeId)} error=${forwardError}`,
        );
      }
    ...
    if (this.pendingManualFloatingSizeChanges.get(activeId) === operation) {
      ...
      if (rectsEqual(observedFrame, targetFrame)) {
        this.acceptPendingManualFloatingSizeChange(operation);
      } else if (
    ...
    if (this.pendingManualFloatingSizeChanges.get(activeId) === operation) {
      this.schedulePendingManualFloatingSizeChangeProbe(operation);
    }

    return operation.status !== "rejected";
  }

  private manualFloatingSizeTarget(
    axis: ManualFloatingResizeAxis,
    ...
    const workArea = command.contextGeometry.workArea;
    const workAreaExtent = workArea[axis];

    if (
      !Number.isFinite(devicePixelRatio) ||
      devicePixelRatio <= 0 ||
      !Number.isFinite(workAreaExtent) ||
      workAreaExtent <= 0
    ) {
      return null;
    }
    ...
    const minimumConstraint =
      axis === "width"
        ? constraintBounds.minimumWidth
        : constraintBounds.minimumHeight;
    const maximumConstraint =
      axis === "width"
        ? constraintBounds.maximumWidth
        : constraintBounds.maximumHeight;
    const minimumExtent = ceilToPhysicalPixel(
      Math.max(minimumConstraint, decorationExtent + 1),
      devicePixelRatio,
    );
    const maximumExtent = Number.isFinite(maximumConstraint)
      ? floorToPhysicalPixel(maximumConstraint, devicePixelRatio)
      : Number.POSITIVE_INFINITY;

    if (
      !Number.isFinite(minimumExtent) ||
      minimumExtent <= decorationExtent ||
    ...
    const resizedFrame =
      axis === "width"
        ? { ...command.originalFrame, width: extent }
  ...
  private handlePendingManualFloatingSizeChange(
    operation: PendingManualFloatingSizeChange,
  ): void {
    if (
      this.pendingManualFloatingSizeChanges.get(operation.command.activeId) !==
      operation
    ) {
      return;
    }
  ...
  private schedulePendingManualFloatingSizeChangeProbe(
    operation: PendingManualFloatingSizeChange,
  ): void {
    if (
      this.pendingManualFloatingSizeChanges.get(operation.command.activeId) !==
    ...
    try {
      this.scheduleResume(() => {
        if (
          !this.started ||
          this.runGeneration !== runGeneration ||
          this.pendingManualFloatingSizeChanges.get(
        ...
        if (
          ...
          operation.settlementAttempts >=
            MAX_MANUAL_FLOATING_SIZE_SETTLEMENT_PROBES
        ) {
          this.finishPendingManualFloatingSizeChange(operation, "rejected");
          return;
        }
      ...
      this.finishPendingManualFloatingSizeChange(operation, "rejected");
      console.warn(
        `[driftile] floating ${operation.axis} resize settlement scheduling failed window=${String(operation.command.activeId)} error=${String(error)}`,
      );
    }
  }

  private acceptPendingManualFloatingSizeChange(
    operation: PendingManualFloatingSizeChange,
  ): void {
    if (!this.pendingManualFloatingSizeChangeIsCurrent(operation)) {
      this.finishPendingManualFloatingSizeChange(operation, "rejected");
      return;
    }
    ...
    if (
      ...
      restoreBaseline.clientFrame[operation.axis] <= 0 ||
      !this.pendingManualFloatingSizeChangeIsCurrent(operation)
    ) {
      this.finishPendingManualFloatingSizeChange(operation, "rejected");
      return;
    ...
    this.finishPendingManualFloatingSizeChange(operation, "accepted");
  }

  private pendingManualFloatingSizeChangeIsCurrent(
    operation: PendingManualFloatingSizeChange,
  ): boolean {
    const command = operation.command;
    ...
    if (
      operation.status !== "pending" ||
      this.pendingManualFloatingSizeChanges.get(command.activeId) !==
        operation ||
    ...
    let constraintBounds: FrameSizeConstraintBounds | null;
    let decorationExtent: number | null;
    let clientExtent: number;

    try {
      constraintBounds = frameSizeConstraintBounds(command.activeWindow);
      decorationExtent = validDecorationExtent(
        command.activeWindow.frameGeometry[operation.axis],
        command.activeWindow.clientGeometry[operation.axis],
      );
      clientExtent = command.activeWindow.clientGeometry[operation.axis];
    } catch {
      return false;
    }
    ...
    return (
      ...
      decorationExtent !== null &&
      nearlyEqual(decorationExtent, operation.decorationExtent) &&
  ...
  private finishPendingManualFloatingSizeChange(
    operation: PendingManualFloatingSizeChange,
    status: "accepted" | "rejected",
  ): void {
    const id = operation.command.activeId;

    if (this.pendingManualFloatingSizeChanges.get(id) !== operation) {
      return;
    }

    this.pendingManualFloatingSizeChanges.delete(id);
    ...
    try {
      ...
      console.warn(
        `[driftile] floating ${operation.axis} resize signal disconnection failed window=${String(id)} error=${String(error)}`,
      );
    }
  }

  private cancelPendingManualFloatingSizeChange(id: WindowId): void {
    const operation = this.pendingManualFloatingSizeChanges.get(id);
  ...
  private clearPendingManualFloatingSizeChanges(): void {
    for (const operation of [
      ...this.pendingManualFloatingSizeChanges.values(),
    ]) {
      this.finishPendingManualFloatingSizeChange(operation, "rejected");
    }
  }
  ...
  private prepareManualFloatingFrameChange(
    ...
    if (this.pendingManualFloatingSizeChangeBlocksFrameChange(activeId)) {
      return null;
    }
  ...
  private pendingManualFloatingSizeChangeBlocksFrameChange(
    id: WindowId,
  ): boolean {
    const operation = this.pendingManualFloatingSizeChanges.get(id);
  ...
  private manualFloatingFrameChangeIsCurrent(
    ...
    if (
      ...
      (pendingSizeChange !== undefined &&
        pendingSizeChange !== allowedPendingSizeChange) ||
  ...
  private moveActiveFloatingWindowToDesktop(
    ...
    if (!follow && (wholeColumn || !manualFloating)) {
      return false;
    }
  ...
  private applyFloatingDesktopTransfer(
    ...
    let frameWrites = 0;
    let failure: string;
    const sourceFocus = command.follow
      ? null
      : this.desktopSendFocusTarget(
          command.sourceContextKey,
          command.sourceLayout,
          new Set([command.activeId]),
        );
    ...
    try {
      ...
      if (!command.follow) {
        if (
          ...
          throw new Error("floating desktop send was not accepted");
        }
  ...
  private moveActiveWindowToDesktop(
    ...
    const floatingResult = this.moveActiveFloatingWindowToDesktop(
      target,
      follow,
      wholeColumn,
    );
    ...
    try {
      transferred = command.follow
        ? this.applyDesktopTransfer(
            command,
            preview,
            sourceLayout,
            targetLayout,
  ...
  private transferLayoutIsSafe(
    ...
    hiddenGeometryIds?: ReadonlySet<WindowId>,
  ): boolean {
    ...
    return layout.windows.every((window) => {
      ...
      if (hiddenGeometryIds?.has(window.windowId) === true) {
        return true;
      }
  ...
  private desktopSendActiveStateIsSafe(
    ...
    if (command.memberIds.has(activeId)) {
      return this.observer.source(activeId) === active;
    }
  ...
  private desktopSendOperationIsCurrent(
    command: DesktopTransferCommand,
    operation: WindowTransferOperation,
    topologyRevision: number,
  ): boolean {
    return (
      ...
      this.desktopSendActiveStateIsSafe(command)
    );
  }

  private applyDesktopSend(
    command: DesktopTransferCommand,
    preview: ContextTransferPreview,
    sourceLayout: ReturnType<typeof solveStripGeometry>,
    targetLayout: ReturnType<typeof solveStripGeometry>,
    ...
    const sourceFocus = this.desktopSendFocusTarget(
      command.contextKey,
      preview.value.sourceLayout,
      command.memberIds,
    );
    const sourceGeometryLayout = sourceLayout.windows.filter(
      (window) => !command.retainedSourceIds.has(window.windowId),
    );
    const sourceWindowIds = sourceGeometryLayout.map(
      (window) => window.windowId,
    );
    const sourceFrames = this.geometry.observedFrames(
      sourceWindowIds,
      command.context,
    );
    const memberSourceFrames = new Map(
      command.members.map((member) => [
        member.id,
        { ...member.window.frameGeometry },
      ]),
    );
    const targetFrames = new Map<WindowId, Rect>();

    for (const window of targetLayout.windows) {
      if (command.memberIds.has(window.windowId)) {
        continue;
      }
      ...
      targetFrames.set(window.windowId, { ...source.frameGeometry });
    }

    if (
      sourceFrames.size !== sourceWindowIds.length ||
      sourceGeometryLayout.some(
        (window) =>
          !this.geometry.canApplyFrame(
            window.windowId,
            window.frame,
    ...
    const changes = diffWindowGeometries(sourceGeometryLayout, sourceFrames);
    const changedWindowIds = new Set(changes.map((change) => change.windowId));
    const rollbackTargets = changes.map((change) => ({
      frame: { ...(sourceFrames.get(change.windowId) ?? change.frame) },
    ...
    try {
      if (
        ...
        throw new Error("desktop send ownership changed");
      }
      ...
      for (const member of this.transferMembersActiveLast(command)) {
        if (
          ...
          throw new Error("desktop send context changed");
        }
        ...
        if (
          ...
          throw new Error("desktop send assignment was rejected");
        }
      }
      ...
      for (const change of changes) {
        if (
          !this.desktopSendOperationIsCurrent(
            command,
            operation,
            topologyRevision,
          ) ||
      ...
      if (
        ...
        throw new Error("desktop send source focus was rejected");
      }
      ...
      if (
        ...
        [...targetFrames].some(([id, frame]) => {
          const source = this.observer.source(id);
          return !source || !rectsEqual(source.frameGeometry, frame);
        }) ||
        !this.desktopSendOperationIsCurrent(
          command,
          operation,
        ...
        throw new Error("desktop send transaction was not accepted");
      }
    ...
    const compensationWrites = this.rollbackDesktopSend(
      command,
      attemptedChanges,
      rollbackTargets,
      memberSourceFrames,
    ...
    this.lastWrites = forwardWrites + compensationWrites;
    console.warn(
      `[driftile] desktop send rolled back window=${String(command.activeId)} error=${failure}`,
    );
    return false;
  }
  ...
  private desktopTransferMechanismAtTarget(
    ...
    return (
      ...
      currentDesktopForOutput(this.workspace, command.output)?.id ===
        (command.follow ? command.targetDesktop.id : command.sourceDesktop.id)
    );
  }
  ...
  private desktopTransferFinalStateIsSafe(
    ...
    return (
      ...
      this.transferLayoutIsSafe(
        ...
        command.contextKey,
        command.contextKey,
        command.follow ? undefined : command.contextKey,
        command.retainedSourceIds,
      ) &&
      this.transferLayoutIsSafe(
        targetLayout,
        ...
        command.targetContextKey,
        command.geometryPassiveIds,
        command.follow ? undefined : command.memberIds,
      )
    );
  }
  ...
  private rollbackDesktopSend(
    command: DesktopTransferCommand,
    forwardChanges: readonly { frame: Rect; windowId: WindowId }[],
    rollbackTargets: readonly { frame: Rect; windowId: WindowId }[],
    memberSourceFrames: ReadonlyMap<WindowId, Rect>,
    mechanismFrames: ReadonlyMap<WindowId, Rect>,
    originalActiveWindow: KWinWindow | null,
    operation: WindowTransferOperation,
    topologyRevision: number,
    ...
    if (compensationSafe) {
      for (const target of [...rollbackTargets].reverse()) {
        if (!attemptedIds.has(target.windowId)) {
          continue;
        }
        ...
        const applied = this.geometry.apply(
          ...
          () =>
            this.transferOperationIdentityIsCurrent(
              ...
              operation,
              topologyRevision,
            ) && this.windowOwnershipClassificationIsCurrent(target.windowId),
        );
      ...
      for (const member of [...command.members].reverse()) {
        if (this.observer.source(member.id) !== member.window) {
          sourceRestored = false;
          this.pendingWindowSyncs.add(member.id);
          continue;
        }

        let restoreMechanismFrame = { ...member.window.frameGeometry };
        ...
        if (windowIsOnDesktop(member.window, command.targetDesktop)) {
          try {
            ...
            console.warn(
              `[driftile] desktop send restore failed window=${String(member.id)} error=${String(error)}`,
            );
          } finally {
            restoreMechanismFrame = { ...member.window.frameGeometry };
          }
        }

        const sourceFrame = memberSourceFrames.get(member.id);
        const liveFrame = member.window.frameGeometry;
        const frameOwned = [
          sourceFrame,
          mechanismFrames.get(member.id),
          restoreMechanismFrame,
        ].some((frame) => frame && rectsEqual(liveFrame, frame));

        if (
          sourceFrame &&
          windowIsOnDesktop(member.window, command.sourceDesktop) &&
          frameOwned &&
          !rectsEqual(liveFrame, sourceFrame) &&
          this.geometry.canApplyFrame(member.id, sourceFrame, command.context)
        ) {
          const applied = this.geometry.apply(
            [{ frame: sourceFrame, windowId: member.id }],
    ...
    const activeOwnedBySend =
      !active ||
      (activeId !== null &&
        (command.memberIds.has(activeId) ||
          this.windowRemovalFocusCandidateLayer(
            activeId,
            active,
            command.contextKey,
    ...
    if (
      activeOwnedBySend &&
      originalActiveWindow &&
      this.observer.source(command.activeId) === originalActiveWindow &&
      windowIsOnDesktop(originalActiveWindow, command.sourceDesktop)
    ) {
      try {
        this.workspace.activeWindow = originalActiveWindow;
      } catch (error) {
        console.warn(
          `[driftile] desktop send focus restore failed window=${String(command.activeId)} error=${String(error)}`,
        );
      }
    }

    if (sourceWasDirty || !sourceRestored) {
      this.markContextDirty(command.sourceRuntimeContext);
    } else {
      this.dirtyContexts.delete(command.contextKey);
    }

    if (targetWasDirty || !sourceRestored) {
      const target = this.contexts.get(command.targetContextKey);

      if (target) {
        this.markContextDirty(target);
      }
    }

    if (this.toggleTransitionPending(command.contextKey)) {
      this.scheduleToggleTransitionProbe(command.contextKey);
    }
    ...
    return intent.participants.every((participant) => {
      ...
      return Boolean(
        ...
        (participant.id === intent.draggedWindowId ||
          (this.windowStateRevisions.get(participant.id) ?? 0) ===
            participant.stateRevision) &&
        (allowInteractiveSource && participant.id === intent.draggedWindowId
          ? this.interactiveMoveSourceIsEligible(participant.window)
          : this.stackTransferMemberIsEligible(
              participant.id,
              participant.window,
              context,
  ...
  private admitTopologyWindowGroup(
    ...
    for (const candidate of candidates) {
      ...
      if (!planned) {
        ...
        const initialFocus =
          metadata === undefined && !preservedRestoreBaselines.has(candidate.id)
            ? this.freshInitialFocus(candidate.id, candidate.source)
            : DEFAULT_INITIAL_FOCUS;
        const initiallyFocused = initialFocus === "focused";
        const initiallyUnfocused = initialFocus === "unfocused";
        const initiallyMaximized =
          metadata === undefined &&
          !preservedRestoreBaselines.has(candidate.id) &&
          this.freshInitialMaximizedApplies(candidate.id, candidate.source);
    ...
    for (const candidate of admittedCandidates) {
      this.finishInitialAdmissionStates(
        candidate.id,
        candidate.source,
  ...
  private admitWindowGroup(
    ...
    let admittedCandidates = candidates.filter((candidate) => {
      ...
      const initialFocus = this.freshInitialFocus(
        candidate.id,
        candidate.source,
      );

      if (initialFocus === "focused") {
        initialFocusedCandidates.add(candidate.id);
      }

      if (initialFocus === "unfocused") {
        initialUnfocusedCandidates.add(candidate.id);
      }
  ...
  private tryAdmitWindow(source: KWinWindow): boolean {
    ...
    const initialFocus = this.freshInitialFocus(id, source);
    const initiallyFocused = initialFocus === "focused";
    const initiallyUnfocused = initialFocus === "unfocused";
  ...
  private applicationInitialFocus(
    source: KWinWindow,
    policy: FreshInitialFocusPolicy,
  ): DefaultInitialFocus {
    if (!source.normalWindow) {
    ...
    return policy.defaultFocus;
  }

  private freshInitialFocus(
    id: WindowId,
    source: KWinWindow,
  ): DefaultInitialFocus {
    const policy = this.initialFocusPolicyByWindow.get(id);

    if (!policy || this.windowAdmissionHistory.has(id)) {
      return DEFAULT_INITIAL_FOCUS;
    }

    return this.applicationInitialFocus(source, policy);
  }
  ...
  private tryFinishInitialFocusedAdmission(
    ...
    const key = contextKey(context);
    const layer = this.focusAvailableWindowLayer(id, source, key);
    ...
    if (this.workspace.activeWindow === source) {
      this.recordWindowFocus(id, source);
      this.rememberLayerFocus(id, source);
      return true;
    }

    if (this.requestWindowFocus(id, source, key, layer)) {
      this.rememberLayerFocus(id, source);
    }
  ...
  private freshInitialMaximizedApplies(
    ...
    const policy = this.initialMaximizedPolicyByWindow.get(id);
    return Boolean(
      policy &&
      !this.windowAdmissionHistory.has(id) &&
      this.applicationInitialMaximizedApplies(source, policy),
    );
  }

  private finishInitialMaximizedAdmission(
    id: WindowId,
    source: KWinWindow,
    applies: boolean,
  ): void {
    ...
    if (
      !applies ||
      source.maximizeMode === 3 ||
      source.maximizable === false ||
      !source.setMaximize ||
      source.deleted ||
      !source.managed
    ) {
    ...
    try {
      source.setMaximize(true, true);
    } catch (error) {
      console.warn(
        `[driftile] initial maximize skipped window=${String(id)} error=${String(error)}`,
      );
    }
  }
  ...
  private markContextDirty(context: RuntimeContext): void {
    this.dirtyContexts.add(context.key);
    this.invalidatePointerExternalPreviewContext(context.key, true);
  }
