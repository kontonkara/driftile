export interface OverviewOutputLabelPlan {
  readonly label: string;
}

// Output badges elide this text; the bound keeps every QML snapshot compact.
...
export function planOverviewOutputLabel(
  output: unknown,
): OverviewOutputLabelPlan | null {
  try {
    if (!isRecord(output)) {
      return null;
    }
