export const APPLICATION_EXCLUSION_LIMITS = Object.freeze({
...
export interface ApplicationExclusions {
  readonly canonicalEntries: readonly string[];
  excludes(desktopFileName: string): boolean;
}

export const EMPTY_APPLICATION_EXCLUSIONS = decodeApplicationExclusions(
  "",
) as ApplicationExclusions;

export function decodeApplicationExclusions(
  value: unknown,
): ApplicationExclusions | null {
  if (
    typeof value !== "string" ||
    value.length > APPLICATION_EXCLUSION_LIMITS.documentCharacters
  ) {
    return null;
  }
  ...
  for (const candidate of candidates) {
    if (candidate.length > APPLICATION_EXCLUSION_LIMITS.rawEntryCharacters) {
      return null;
    }
    ...
    if (identifiers.length >= APPLICATION_EXCLUSION_LIMITS.entries) {
      return null;
    }
...
export function sameApplicationExclusions(
  left: ApplicationExclusions,
  right: ApplicationExclusions,
): boolean {
  if (left === right) {
    return true;
  }
...
function parseIdentifier(value: unknown): string | null {
  if (
    typeof value !== "string" ||
    value.length > APPLICATION_EXCLUSION_LIMITS.rawEntryCharacters
  ) {
    return null;
  }
  ...
  if (
    ...
    identifierBytes > APPLICATION_EXCLUSION_LIMITS.identifierBytes ||
