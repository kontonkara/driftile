...
KWin owns minimization; Driftile provides no minimize action or default shortcut. A minimized tiled window keeps its exact logical slot, while a minimized manually floating window keeps its exact floating frame. Focus navigation and layer switching skip minimized windows and fully minimized columns without wrapping. Native fullscreen and maximize can extract the active stack member past settled minimized peers without writing their frames; other all-member commands remain unavailable while one is minimized.
