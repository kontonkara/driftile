...
KWin owns minimization; Driftile provides no minimize action or default shortcut. A minimized tiled window keeps its exact logical slot, while a minimized manually floating window keeps its exact floating frame. Focus navigation and layer switching skip minimized windows and fully minimized columns without wrapping. A visible stack member can move across settled minimized slots or extract horizontally past those peers without writing hidden frames. Native fullscreen and maximize may also extract it past minimized peers. Other all-member commands remain unavailable while one is minimized.
