...
The current prototype models eligible normal windows in every existing `(output, desktop)` context and applies geometry only to the desktop visible on each output. Focus navigation stays within its context. On single-output setups, `Meta+Ctrl+H` and `Meta+Ctrl+L` reveal adjacent columns with the smallest required scroll. With multiple outputs, windows that would cause horizontal overflow wait unmanaged until their context has capacity. External output and desktop transfers are deferred to the next work batch and re-owned by the destination context. Fullscreen, minimized, maximized, and native-tiled windows keep their layout slot while KWin controls their geometry. Original frames of writable managed windows are restored when the script stops if the output topology is unchanged. Interactive layout editing, topology recovery, dynamic workspace creation, and persistence are not implemented yet.
