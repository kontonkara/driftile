...
KWin-decorated application windows are borderless by default, including tiled, floating, dialog, transient, and utility windows. The setting is optional, never claims windows that were already borderless, and restores owned decorations when disabled or unloaded. Tiled window gaps are configurable live from 0 to 64 logical pixels and default to 16. Newly admitted columns use a 50% proportional width by default, configurable from 10% to 100%. See [Configuration](docs/configuration.md).
