...
Driftile 1.48.0 is the latest stable release. It requires KDE Plasma with KWin
...
- `driftile-transitions-1.48.0.kwineffect` if using optional geometry
  transitions
- `SHA256SUMS`
- `LICENSE`
...
kpackagetool6 --type=KWin/Script \
  --install ./driftile-1.48.0.kwinscript
```
...
kpackagetool6 --type=KWin/Effect \
  --install ./driftile-transitions-1.48.0.kwineffect
```
...
Driftile works without the companion helper. The 1.48.0 helper claims the
...
```bash
node ./driftile-shortcuts-1.48.0.mjs claim
node ./driftile-shortcuts-1.48.0.mjs check
```
...
```bash
node ./driftile-shortcuts-1.48.0.mjs release
```
...
```bash
node ./driftile-shortcuts-1.48.0.mjs claim --profile ./shortcuts.json
node ./driftile-shortcuts-1.48.0.mjs check --profile ./shortcuts.json
node ./driftile-shortcuts-1.48.0.mjs release
```
...
```nix
inputs.driftile.url = "github:kontonkara/driftile/v1.48.0";
```
...
The 1.48.0 module exposes the optional overview as a separate package. It
...
The 1.48.0 Home Manager module can also generate a custom shortcut profile:
