...
Driftile 1.61.0 is the latest stable release. It requires KDE Plasma with KWin
...
[`v1.61.0` release](https://github.com/kontonkara/driftile/releases/tag/v1.61.0):

- `driftile-1.61.0.kwinscript`
- `driftile-overview-1.61.0.kwineffect` if using the optional overview
- `driftile-transitions-1.61.0.kwineffect` if using optional geometry
  transitions
- `SHA256SUMS`
- `LICENSE`
- `driftile-shortcuts-1.61.0.mjs` if using the optional shortcut helper
...
kpackagetool6 --type=KWin/Script \
  --install ./driftile-1.61.0.kwinscript
```
...
kpackagetool6 --type=KWin/Effect \
  --install ./driftile-transitions-1.61.0.kwineffect
```
...
Driftile works without the companion helper. The 1.61.0 helper claims the
...
```bash
node ./driftile-shortcuts-1.61.0.mjs claim
node ./driftile-shortcuts-1.61.0.mjs check
```
...
```bash
node ./driftile-shortcuts-1.61.0.mjs release
```
...
```bash
node ./driftile-shortcuts-1.61.0.mjs claim --profile ./shortcuts.json
node ./driftile-shortcuts-1.61.0.mjs check --profile ./shortcuts.json
node ./driftile-shortcuts-1.61.0.mjs release
```
...
Driftile 1.61.0 provides **Driftile Shortcuts**, an optional Qt/KDE
...
lightweight. To build it from a 1.61.0 checkout, install CMake 3.22 or
...
The 1.61.0 package installs a freedesktop launcher and AppStream metadata,
...
```nix
inputs.driftile.url = "github:kontonkara/driftile/v1.61.0";
```
...
The 1.61.0 module exposes the optional overview as a separate package. It
...
`overview.showApplicationIcons`, and the development
`overview.showOutputNames` option follow the same nullable semantics. The
effect enables window labels, application identity, close buttons, state
...
The 1.61.0 Home Manager module can also generate a custom shortcut profile:
