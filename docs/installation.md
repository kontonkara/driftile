...
Driftile 1.77.0 is the latest stable release. It requires KDE Plasma with KWin
...
[`v1.77.0` release](https://github.com/kontonkara/driftile/releases/tag/v1.77.0):

- `driftile-1.77.0.kwinscript`
- `driftile-overview-1.77.0.kwineffect` if using the optional overview
- `driftile-transitions-1.77.0.kwineffect` if using optional geometry
  transitions
- `SHA256SUMS`
- `LICENSE`
- `driftile-shortcuts-1.77.0.mjs` if using the optional shortcut helper
...
kpackagetool6 --type=KWin/Script \
  --install ./driftile-1.77.0.kwinscript
```
...
kpackagetool6 --type=KWin/Effect \
  --install ./driftile-overview-1.77.0.kwineffect
```
...
```bash
kpackagetool6 --type=KWin/Effect \
  --install ./driftile-transitions-1.77.0.kwineffect
```
...
Driftile works without the companion helper. The 1.77.0 helper claims the
...
```bash
node ./driftile-shortcuts-1.77.0.mjs claim
node ./driftile-shortcuts-1.77.0.mjs check
```
...
```bash
node ./driftile-shortcuts-1.77.0.mjs release
```
...
```bash
node ./driftile-shortcuts-1.77.0.mjs claim --profile ./shortcuts.json
node ./driftile-shortcuts-1.77.0.mjs check --profile ./shortcuts.json
node ./driftile-shortcuts-1.77.0.mjs release
```
...
lightweight. To build it from a 1.77.0 checkout, install CMake 3.22 or
...
```nix
inputs.driftile.url = "github:kontonkara/driftile/v1.77.0";
```
