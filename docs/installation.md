...
Driftile 1.19.0 is the latest stable release. It requires KDE Plasma with KWin
...
[`v1.19.0` release](https://github.com/kontonkara/driftile/releases/tag/v1.19.0):

- `driftile-1.19.0.kwinscript`
- `driftile-overview-1.19.0.kwineffect` if using the optional overview
- `SHA256SUMS`
- `LICENSE`
- `driftile-shortcuts-1.19.0.mjs` if using the optional shortcut helper
...
kpackagetool6 --type=KWin/Script \
  --install ./driftile-1.19.0.kwinscript
```
...
Driftile works without the companion helper. The 1.19.0 helper claims the
...
```bash
node ./driftile-shortcuts-1.19.0.mjs claim
node ./driftile-shortcuts-1.19.0.mjs check
```
...
```bash
node ./driftile-shortcuts-1.19.0.mjs release
```
...
```bash
node ./driftile-shortcuts-1.19.0.mjs claim --profile ./shortcuts.json
node ./driftile-shortcuts-1.19.0.mjs check --profile ./shortcuts.json
node ./driftile-shortcuts-1.19.0.mjs release
```
...
```nix
inputs.driftile.url = "github:kontonkara/driftile/v1.19.0";
```
...
The 1.19.0 module exposes the optional overview as a separate package. It
...
The 1.19.0 Home Manager module can also generate a custom shortcut profile:
...
[1.19.0 release notes](release-notes-1.19.0.md).
