...
Version 1.6.0-rc.1 targets Wayland, XWayland, and a single-output native X11
session. Touchpad navigation is available only on native Wayland. Run all
commands as the desktop user, not with `sudo`.

## Install a release

Download these files from the same release:

- `driftile-1.6.0-rc.1.kwinscript`
- `driftile-overview-1.6.0-rc.1.kwineffect` if using the optional overview
- `SHA256SUMS`
- `LICENSE`
- `driftile-shortcuts-1.6.0-rc.1.mjs` if using the optional shortcut helper
...
kpackagetool6 --type=KWin/Script \
  --install ./driftile-1.6.0-rc.1.kwinscript
```
...
Driftile works without the companion helper. The 1.6.0-rc.1 helper claims the
...
```bash
node ./driftile-shortcuts-1.6.0-rc.1.mjs claim
node ./driftile-shortcuts-1.6.0-rc.1.mjs check
```
...
```bash
node ./driftile-shortcuts-1.6.0-rc.1.mjs release
```
...
```bash
node ./driftile-shortcuts-1.6.0-rc.1.mjs claim --profile ./shortcuts.json
node ./driftile-shortcuts-1.6.0-rc.1.mjs check --profile ./shortcuts.json
node ./driftile-shortcuts-1.6.0-rc.1.mjs release
```
...
```nix
inputs.driftile.url = "github:kontonkara/driftile/v1.6.0-rc.1";
```
...
The 1.6.0-rc.1 module exposes the optional overview as a separate package. It
...
The 1.6.0-rc.1 Home Manager module can also own the complete ten-setting
profile:
...
installation generations. Candidate details are in the
[1.6.0-rc.1 release notes](release-notes-1.6.0-rc.1.md).
