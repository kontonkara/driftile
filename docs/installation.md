...
Driftile 1.34.0 is the latest stable release. It requires KDE Plasma with KWin
...
[`v1.34.0` release](https://github.com/kontonkara/driftile/releases/tag/v1.34.0):

- `driftile-1.34.0.kwinscript`
- `driftile-overview-1.34.0.kwineffect` if using the optional overview
- `driftile-transitions-1.34.0.kwineffect` if using optional geometry
  transitions
- `SHA256SUMS`
- `LICENSE`
...
kpackagetool6 --type=KWin/Script \
  --install ./driftile-1.34.0.kwinscript
```
...
kpackagetool6 --type=KWin/Effect \
  --install ./driftile-transitions-1.34.0.kwineffect
```
...
Driftile works without the companion helper. The 1.34.0 helper claims the
...
```bash
node ./driftile-shortcuts-1.34.0.mjs claim
node ./driftile-shortcuts-1.34.0.mjs check
```
...
```bash
node ./driftile-shortcuts-1.34.0.mjs release
```
...
```bash
node ./driftile-shortcuts-1.34.0.mjs claim --profile ./shortcuts.json
node ./driftile-shortcuts-1.34.0.mjs check --profile ./shortcuts.json
node ./driftile-shortcuts-1.34.0.mjs release
```
...
```nix
inputs.driftile.url = "github:kontonkara/driftile/v1.34.0";
```
...
The 1.34.0 module exposes the optional overview as a separate package. It
...
The 1.34.0 Home Manager module can also generate a custom shortcut profile:
