...
- Stacked native-state extraction may pass settled minimized peers, retaining their exact slots without frame writes. Whole-column desktop transfers may carry settled minimized passive members without writing their frames and must preserve their logical slots, height state, minimized state, and column width. Output transfers still require every participant to be writable; other hidden-member edit semantics remain MVP work.
