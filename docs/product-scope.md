...
- Explicit consume appends the immediate right column's visible top member to the active column; explicit expel moves the active column's bottom member into a new right column. Focus remains in the active column.
...
- Stacked native-state extraction may pass settled minimized peers, retaining their exact slots without frame writes. Whole-column transfers and expel remain unavailable when a participating member is minimized; other hidden-member edit semantics remain MVP work.
