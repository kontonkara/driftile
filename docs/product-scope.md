...
- Stacked native-state extraction may pass settled minimized peers, retaining their exact slots without frame writes. Whole-column desktop and output transfers may carry settled minimized passive members without layout geometry writes and must preserve their logical slots, height state, minimized state, and column width. Secondary single-window transfers and other hidden-member edit semantics remain fail-closed MVP work.
