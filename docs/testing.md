...
The test uses temporary user and XDG directories, private D-Bus sessions, virtual Wayland outputs, and an Xvfb display. Its single-output sessions verify three-window tiling, viewport reveal, vertical stack focus and editing, direct insertion across singleton columns, desktop-transfer round trips, output-transfer boundaries, manual floating round trips, automatic KWin ownership for dialogs and fixed-size normal windows, active-column reorder and resizing through KGlobalAccel, exact restoration, package removal, and slot preservation through fullscreen, minimize, and maximize transitions with native Wayland, Xwayland, and native X11 windows. Native tiling is also covered on Wayland and Xwayland. A separate two-output Wayland session enables KWin's independent desktop mode and verifies desktop and directional output transfers, output-local stack insertion and floating round trips, exact geometry, overflow rejection, and safe unload ownership.

The unit suite covers stack mutations, deterministic output routing, two-context desktop and output transfer commits and rollback, manual-floating placement anchors and transactional ownership, automatic KWin ownership, signaled and silent reclassification, late-role readmission, decorated client-to-frame constraints, active-column reorder, width constraints and rollback, the two-sample topology barrier, output and dock invalidations, waiting-only and hidden contexts, deterministic structural merges, sticky restore invalidation, capacity eviction and retry, and stale callback cancellation.
...
The script builds `nixosConfigurations.driftile-vm` through `nixos-rebuild build-vm` and asks host KWin for a centered `1440x900` QEMU window with a `1680x1050` guest display. Plasma starts a Wayland session, enables Driftile, and opens labeled test windows. The launcher verifies extension loading, desktop-transfer round trips, one-output transfer boundaries, four-way focus, stack editing, direct stack insertion, floating round trips, active-column movement and width shortcuts, and viewport scrolling. Use `Meta+Ctrl+H/J/K/L` to focus, `Meta+Ctrl+Shift+H/L` to move columns, `Meta+Ctrl+Shift+J/K` to reorder stack members, `Meta+Ctrl+Alt+H/L` to merge or extract a window, `Meta+Ctrl+Alt+Shift+H/L` to insert into the nearest stack, `Meta+Ctrl+Alt+K/J` to move a window to the previous or next desktop, `Meta+Ctrl+Alt+Shift+Arrow` to move it to an adjacent output, `Meta+Ctrl+Space` to toggle floating, and `Meta+Ctrl+-/=/0` to change width.
