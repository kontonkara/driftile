...
layout work; explicit manual-floating movement, centering, or contextual width
...
Manual-floating width coverage verifies configured work-area steps, decorated
live minimum and maximum bounds, positive client width, physical-pixel snapping,
partial visibility, and exactly one forward request. It covers immediate and
delayed exact acknowledgement, repeated-command serialization with movement and
centering, bounded expiry of an unchanged request, shutdown cleanup, exact
