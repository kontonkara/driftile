...
The test uses temporary user and XDG directories, private D-Bus sessions, virtual Wayland outputs, and an Xvfb display. Its single-output sessions verify three-window tiling, viewport reveal, active-column reorder and resizing through KGlobalAccel, exact restoration, package removal, and slot preservation through fullscreen, minimize, and maximize transitions with native Wayland, Xwayland, and native X11 windows. Native tiling is also covered on Wayland and Xwayland. A separate two-output Wayland session verifies independent contexts, exact output-local geometry, overflow rejection, and safe unload ownership.

The unit suite covers active-column reorder, width constraints and rollback, the two-sample topology barrier, output and dock invalidations, waiting-only and hidden contexts, deterministic structural merges, sticky restore invalidation, capacity eviction and retry, and stale callback cancellation.
...
The script builds `nixosConfigurations.driftile-vm` through `nixos-rebuild build-vm` and asks host KWin for a centered `1440x900` QEMU window with a `1680x1050` guest display. Plasma starts a Wayland session, enables Driftile, and opens three labeled test windows. The launcher verifies extension loading, horizontal focus, active-column movement and width shortcuts, and viewport scrolling. Use `Meta+Ctrl+H/L` to focus, `Meta+Ctrl+Shift+H/L` to reorder, and `Meta+Ctrl+-/=/0` to decrease, increase, or reset width.
