...
- Pin the active full-width frame between equal configured outer gaps, place adjacent frames at least one physically aligned configured gap beyond the corresponding viewport edge, keep an inactive full-width frame outside that edge, add no clearance when the gap is zero, and restore its prior width and viewport in one transaction. Derive this clearance without adding state, schema, or bindings.
