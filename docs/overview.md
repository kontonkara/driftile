...
The 1.14.0 release does not change overview behavior.
...
The [1.14.0 package](release-notes-1.14.0.md) is the latest stable release.
Download `driftile-overview-1.14.0.kwineffect` and `SHA256SUMS` from that
...
kpackagetool6 --type=KWin/Effect \
  --install ./driftile-overview-1.14.0.kwineffect
```
...
The 1.14.0 flake exposes the effect separately as
