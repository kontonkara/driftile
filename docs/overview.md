...
to small scenes and scrolls when needed. A mouse wheel or touchpad scroll,
`Up`/`Down`, `Page Up`/`Page Down`, and `Home`/`End` move the open panel within
its bounds without changing the Overview selection. On sufficiently large
scenes, its hint also opens the panel with a left click or touch and hides
during search or while the panel is open. While it is open, background window,
