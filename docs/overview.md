...
The 1.11.0 release does not change overview behavior.
...
The [1.11.0 package](release-notes-1.11.0.md) is the latest stable release.
Download `driftile-overview-1.11.0.kwineffect` and `SHA256SUMS` from the same
...
kpackagetool6 --type=KWin/Effect \
  --install ./driftile-overview-1.11.0.kwineffect
```
...
The 1.11.0 flake exposes the effect separately as
