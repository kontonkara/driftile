...
The 1.9.1 release does not change overview behavior.
...
Download `driftile-overview-1.9.1.kwineffect` and `SHA256SUMS` from the
[latest stable release](release-notes-1.9.1.md), then verify the archive:
...
kpackagetool6 --type=KWin/Effect \
  --install ./driftile-overview-1.9.1.kwineffect
```
...
The 1.9.1 flake exposes the effect separately as
