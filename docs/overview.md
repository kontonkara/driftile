...
The 1.19.0-rc.1 candidate projects only the selected member's thumbnail from a
tabbed column. The companion does not expose hidden tab members or provide tab
controls.
...
The [1.19.0-rc.1 package](release-notes-1.19.0-rc.1.md) is the current candidate
and is not a stable release; 1.18.0 remains the latest stable version. Download
`driftile-overview-1.19.0-rc.1.kwineffect` and `SHA256SUMS` from the same release,
...
kpackagetool6 --type=KWin/Effect \
  --install ./driftile-overview-1.19.0-rc.1.kwineffect
```
...
The 1.19.0-rc.1 flake exposes the effect separately as
