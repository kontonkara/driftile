...
The 1.13.0-rc.1 candidate does not change overview behavior.
...
The [1.13.0-rc.1 package](release-notes-1.13.0-rc.1.md) is the current candidate
and is not a stable release; 1.12.0 remains the latest stable version. Download
`driftile-overview-1.13.0-rc.1.kwineffect` and `SHA256SUMS` from the same release,
...
kpackagetool6 --type=KWin/Effect \
  --install ./driftile-overview-1.13.0-rc.1.kwineffect
```
...
The 1.13.0-rc.1 flake exposes the effect separately as
