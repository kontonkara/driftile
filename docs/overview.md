...
Download `driftile-overview-1.49.0.kwineffect` and `SHA256SUMS` from the stable
[1.49.0 release](release-notes-1.49.0.md), then verify the archive:
...
kpackagetool6 --type=KWin/Effect \
  --install ./driftile-overview-1.49.0.kwineffect
```
...
The 1.49.0 flake exposes the effect separately as
...
Version 1.49.0 validates its gesture, same- and cross-output transfer, search,
