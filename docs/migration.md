...
3. Install matching 1.83.0 artifacts, or pin the Nix input to `v1.83.0` and
   rebuild.
...
The optional Overview adds right-button row panning, keeps a window drag active
while a bounded dwell selects another workspace, and refreshes in place when
virtual desktops are added, removed, or reordered. The normal layout remains
authoritative, and stale pointer or workspace identities fail closed.

Overview remains optional and under active development.

## Roll back from 1.83.0 to 1.82.0

Release a helper-owned profile, disable Driftile and both optional effects,
then restore matching verified 1.82.0 artifacts or pin the Nix input to
`v1.82.0` and rebuild. Logical persistence v4 remains compatible; no option
deletion or schema migration is required. The older Overview closes when the
virtual-desktop topology changes and does not provide dwell-based drag
continuation between workspace rows.

## Upgrade from 1.81.0 to 1.82.0

1. Release a helper-owned shortcut profile with the installed helper.
2. Disable Driftile and both optional effects in System Settings.
3. Install matching 1.82.0 artifacts, or pin the Nix input to `v1.82.0` and
   rebuild.
4. Re-enable Driftile and only the optional effects you use, then reclaim the
   helper profile if needed.

No shortcut, setting, schema, layout, or persistence migration is required.
The optional Overview now presents full-width workspace rows in one continuous
spatial plane. It projects exact tiled and floating geometry, supports
reversible open and close motion, and keeps two-axis pointer, wheel, and
keyboard navigation session-local. Empty and floating-only rows fail safely.
The effect avoids unsafe weak-collection query caches and repeated KWin
background items while retaining fail-closed live-target validation.

Overview remains an optional projection. The normal Driftile layout stays
authoritative and no Overview interaction writes layout ownership or persisted
viewport state.

## Roll back from 1.82.0 to 1.81.0

Release a helper-owned profile, disable Driftile and both optional effects,
then restore matching verified 1.81.0 artifacts or pin the Nix input to
`v1.81.0` and rebuild. Logical persistence v4 remains compatible; no option
deletion or schema migration is required. The older Overview returns to the
intermediate row projection described by its release notes.
...
1. Release a helper-owned shortcut profile with the installed helper.
2. Disable Driftile and both optional effects in System Settings.
3. Install matching 1.79.0 artifacts, or pin the Nix input to `v1.79.0` and
   rebuild.
4. Re-enable Driftile and only the optional effects you use, then reclaim the
   helper profile if needed.

No shortcut, setting, schema, layout, or persistence migration is required.
The current Overview row can now follow the live frame geometry of its active
eligible tiled window. Inactive rows remain captured, session-only
projections.

## Roll back from 1.79.0 to 1.78.0

Release a helper-owned profile, disable Driftile and both optional effects,
then restore matching verified 1.78.0 artifacts or pin the Nix input to
`v1.78.0` and rebuild. Logical persistence v4 remains compatible; no option
deletion or schema migration is required.
...
1. Release a helper-owned shortcut profile with the installed helper.
2. Disable Driftile and both optional effects in System Settings.
3. Install matching 1.77.0 artifacts, or pin the Nix input to `v1.77.0` and
   rebuild.
4. Re-enable Driftile and only the optional effects you use, then reclaim the
   helper profile if needed.

No shortcut, setting, layout, or persistence migration is required. Overview
workspaces now use wide spatial rows with visible neighboring columns,
session-only horizontal viewports, and horizontal wheel and keyboard
navigation.

## Roll back from 1.77.0 to 1.76.0

Release a helper-owned profile, disable Driftile and both optional effects,
then restore matching verified 1.76.0 artifacts or pin the Nix input to
`v1.76.0` and rebuild. Logical persistence v4 remains compatible; no option
deletion or schema migration is required.

## Upgrade from 1.75.0 to 1.76.0

1. Release a helper-owned shortcut profile with the installed helper.
2. Disable Driftile and both optional effects in System Settings.
3. Install matching 1.76.0 artifacts, or pin the Nix input to `v1.76.0` and
   rebuild.
4. Re-enable Driftile and only the optional effects you use, then reclaim the
   helper profile if needed.

No shortcut, setting, layout, or persistence migration is required. The
Overview keeps its camera position across live refreshes, zoom changes, and
scene resizes; background mouse panning and precise wheel movement are
available. Geometry transitions now survive temporary workspace-effect
ownership.

## Roll back from 1.76.0 to 1.75.0

Release a helper-owned profile, disable Driftile and both optional effects,
then restore matching verified 1.75.0 artifacts or pin the Nix input to
`v1.75.0` and rebuild. Logical persistence v4 remains compatible; no option
deletion or schema migration is required.
...
1. Release a helper-owned shortcut profile with the installed helper.
2. Disable Driftile and both optional effects in System Settings.
3. Install matching 1.74.2 artifacts, or pin the Nix input to `v1.74.2` and
   rebuild.
4. Re-enable Driftile and only the optional effects you use, then reclaim the
   helper profile if needed.

No shortcut, setting, layout, or persistence migration is required. The update
replays one immediate horizontal focus command after a newly selected desktop
publishes its tiled active window and prevents partial Overview wheel input
from crossing a spatial pan or workspace change.

## Roll back from 1.74.2 to 1.74.1

Release a helper-owned profile, disable Driftile and both optional effects,
then restore matching verified 1.74.1 artifacts or pin the Nix input to
`v1.74.1` and rebuild. No option deletion or schema migration is required.
...
1. Release a helper-owned shortcut profile with the installed helper.
2. Disable Driftile and both optional effects in System Settings.
3. Install matching 1.68.0 artifacts, or pin the Nix input to `v1.68.0` and
   rebuild.
4. Re-enable Driftile and only the optional effects you use, then reclaim the
   helper profile if needed.

Logical layout state remains v4. Moving columns now retargets position when an
application corrects its size during the animation, preventing one window from
remaining visually separated from its neighbors. Close-focus recovery also
survives a provisional replacement followed by a cleared activation. No
setting, default binding, or schema migration is required.

## Roll back from 1.68.0 to 1.67.0

Release a helper-owned profile, disable Driftile and both optional effects,
then restore matching verified 1.67.0 artifacts or pin the Nix input to
`v1.67.0` and rebuild. No option deletion or schema migration is required; the
transition and focus corrections simply become unavailable.
...
1. Release a helper-owned shortcut profile with the installed helper.
2. Disable Driftile and both optional effects in System Settings.
3. Install matching 1.65.0 artifacts, or pin the Nix input to `v1.65.0` and
   rebuild.
4. Re-enable Driftile and only the optional effects you use, then reclaim the
   helper profile if needed.

Logical layout state remains v4. The optional Overview adds up to four bounded
search alternatives separated by a standalone unquoted `|`. `Ctrl+Backspace`
removes the trailing structured clause and `Ctrl+U` clears a non-empty query.
These shortcuts are active only while the Overview is open. No setting or
schema migration is required.

## Roll back from 1.65.0 to 1.64.0

Release a helper-owned profile, disable Driftile and both optional effects,
then restore matching verified 1.64.0 artifacts or pin the Nix input to
`v1.64.0` and rebuild. No option deletion or schema migration is required; the
alternative-group syntax and search-editing shortcuts simply become
unavailable.
...
1. Release a helper-owned shortcut profile with the installed helper.
2. Disable Driftile and both optional effects in System Settings.
3. Install matching 1.63.0 artifacts, or pin the Nix input to `v1.63.0` and
   rebuild.
4. Re-enable Driftile and only the optional effects you use, then reclaim the
   helper profile if needed.

Logical layout state remains v4. The optional Overview adds exact per-desktop
match counts, selected-result ordinal and total feedback, and static deemphasis
for desktop cards with zero results. Settings, schemas, layout, persistence,
actions, bindings, and window input paths are unchanged.

## Roll back from 1.63.0 to 1.62.0

Release a helper-owned profile, disable Driftile and both optional effects,
then restore matching verified 1.62.0 artifacts or pin the Nix input to
`v1.62.0` and rebuild. No option deletion or schema migration is required; the
new Overview search feedback simply disappears.
...
1. Release a helper-owned shortcut profile with the installed helper.
2. Disable Driftile and both optional effects in System Settings.
3. Install matching 1.61.0 artifacts, or pin the Nix input to `v1.61.0` and
   rebuild.
4. Re-enable Driftile and only the optional effects you use, then reclaim the
   helper profile if needed.

Logical layout state remains v4. The optional Overview adds bounded desktop
names and lazy application icons; both presentations default to enabled and
can be managed with nullable Home Manager options. Layout, persistence,
actions, bindings, and window input paths are unchanged.

## Roll back from 1.61.0 to 1.59.0

Release a helper-owned profile, disable Driftile and both optional effects,
then restore matching verified 1.59.0 artifacts or pin the Nix input to
`v1.59.0` and rebuild. Remove `overview.showDesktopNames` and
`overview.showApplicationIcons` before evaluating the 1.59.0 Home Manager
module. Retained KConfig values are ignored by the older Overview.
