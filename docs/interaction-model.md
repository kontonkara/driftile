...
fields and operators. On sufficiently large idle scenes, the F1 hint also opens
the reference with a left click or touch. A new overview session starts with
the reference hidden.
...
| Virtual desktops     | Focus adjacent or numbered desktops; reorder adjacently or by position; transfer a column or window | Available |
