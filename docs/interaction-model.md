...
| Area                 | Behavior                                                                                            |
| -------------------- | --------------------------------------------------------------------------------------------------- |
| Horizontal strip     | Focus and reorder columns; focus or move to first and last; reveal with minimal scrolling           |
| Vertical column      | Focus and reorder members; consume or expel active, top, or bottom members                          |
| Column view          | Cycle configured widths in both directions; adjust by 10%; toggle full width; center                |
| Advanced column view | Fill available width and center all fully visible columns                                           |
| Window height        | Adjust one window by 10%; reset to automatic; cycle configured presets                              |
| Virtual desktops     | Focus adjacent or numbered desktops; reorder adjacently or by position; transfer a column or window |
| Outputs              | Focus an adjacent output and transfer the active column or floating window                          |
| Fullscreen           | Extract a regular stack member, then toggle native fullscreen through KWin                          |
| Native maximize      | Extract a regular stack member, then toggle it to work-area edges through KWin                      |
| Minimize focus       | Preserve tiled slots and floating frames; skip minimized windows without wrapping                   |
| Hidden-member edits  | Preserve passive peers and reject unsafe minimized-member structural edits                          |
| Floating layer       | Toggle state, switch layers, navigate geometrically, nudge, center, and resize contextually         |
| Pointer drop         | Reinsert a tiled window or tile a manual floating window at an exact window or empty gutter         |
| Pointer resize       | Adopt one completed horizontal column resize or vertical stacked-window resize                      |
| Overview companion   | Filter or activate windows; select or reorder desktops; move windows between desktop rows           |
| Tabbed columns       | Toggle presentation; select or reorder members with the existing vertical grammar                   |
| Pointer navigation   | Pan workspace rows on both wheel axes; switch workspaces; cycle matching search results             |
