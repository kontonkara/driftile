  ...
  for ((attempt = 0; attempt < 600; attempt += 1)); do
    if [[ -f "$loaded_file" && -f "$focus_file" ]]; then
      ...
      if [[ "$(<"$focus_file")" == true ]]; then
        printf 'The VM verified desktop and output boundaries, focus, floating, direct stack insertion, stack editing, column resizing, and viewport scrolling.\n'
      else
        printf 'The VM failed to verify desktop and output boundaries, focus, floating, direct stack insertion, stack editing, column resizing, and viewport scrolling.\n' >&2
        failed=true
