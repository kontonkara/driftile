...
> = {
  ...
  driftile_center_window: {
    activated: "Runtime.DriftileRuntime.centerWindow()",
  },
  ...
  driftile_move_desktop_to_index_1: {
    activated: "Runtime.DriftileRuntime.moveDesktopToIndex(1)",
  },
  driftile_move_desktop_to_index_2: {
    activated: "Runtime.DriftileRuntime.moveDesktopToIndex(2)",
  },
  driftile_move_desktop_to_index_3: {
    activated: "Runtime.DriftileRuntime.moveDesktopToIndex(3)",
  },
  driftile_move_desktop_to_index_4: {
    activated: "Runtime.DriftileRuntime.moveDesktopToIndex(4)",
  },
  driftile_move_desktop_to_index_5: {
    activated: "Runtime.DriftileRuntime.moveDesktopToIndex(5)",
  },
  driftile_move_desktop_to_index_6: {
    activated: "Runtime.DriftileRuntime.moveDesktopToIndex(6)",
  },
  driftile_move_desktop_to_index_7: {
    activated: "Runtime.DriftileRuntime.moveDesktopToIndex(7)",
  },
  driftile_move_desktop_to_index_8: {
    activated: "Runtime.DriftileRuntime.moveDesktopToIndex(8)",
  },
  driftile_move_desktop_to_index_9: {
    activated: "Runtime.DriftileRuntime.moveDesktopToIndex(9)",
  },
  ...
  driftile_move_window_to_tiling: {
    activated: "Runtime.DriftileRuntime.moveWindowToTiling()",
  },
  ...
  driftile_switch_preset_window_width: {
    activated: "Runtime.DriftileRuntime.switchPresetWindowWidth()",
  },
  driftile_switch_preset_window_width_back: {
    activated: "Runtime.DriftileRuntime.switchPresetWindowWidthBack()",
  },
...
describe("KWin shortcut handlers", () => {
  ...
  it("exposes exact application admission rules as bounded lists", () => {
    ...
    const initialFocusedEntry = configuration.match(
      /<entry name="ApplicationInitialFocused" type="String">([\s\S]*?)<\/entry>/,
    )?.[1];
    const initialFocusedLabel = configurationUi.match(
      /<widget class="QLabel" name="applicationInitialFocusedLabel">([\s\S]*?)<\/widget>/,
    )?.[1];
    ...
    expect(initialFocusedLabel).toContain(
      "<string>Applications initially focused:</string>",
    );
    expect(initialFocusedLabel).toContain(
    ...
    expect(initialFocusedWidget).toContain(
      "The rule never selects a desktop or output and does not suppress normal KWin focus for unlisted applications.",
    );
    expect(initialFocusedWidget).toContain(
      "Rejected or unavailable focus requests are not retried.",
    );
    expect(initialFocusedWidget).toContain(
      "Live edits affect future tracked windows only.",
    );
