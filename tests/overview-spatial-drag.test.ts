...
const thumbnailDrag = handlerBlock("thumbnailDragHandler", "tabShell");
const tabDrag = handlerBlock("tabDragHandler", "minimizedPlaceholderShell");
...
describe("spatial overview window drag lifecycle", () => {
  it("declares one deduplicated source lifecycle without changing minimized input", () => {
    ...
    expect(tabDrag).toContain("!windowPresentation.minimizedWindow");
    expect(
      desktopCard.slice(
  ...
  it.each([
    ["thumbnail", "thumbnailDragHandler", thumbnailDrag],
    ["tab", "tabDragHandler", tabDrag],
  ])(
    "emits a complete %s drag lifecycle from the existing handler",
    (_, handlerId, drag) => {
